...
**用于审计、简化、实现和验证可靠响应式 Web 界面的可移植 Agent Skill。**
...
仓库包含一个框架无关的标准 Agent Skill，以及一层轻量 Codex Plugin 封装。Skill 始终是工作流的唯一事实来源。
...
- 根据用户授权自动区分只读审计、实施修改和验证模式。
- 检查容器、Grid、Flex、内在尺寸、溢出、缩放、媒体、表格和视口行为。
- 使用控件、按钮、卡片、面板、重点展示、胶囊和圆形语义建立圆角层级。
...
面向人的项目说明放在仓库根目录；Skill 内只保留 Agent 执行任务真正需要的指令和资源。

## 安装

### 通用 Agent Skills 客户端

按照客户端支持的 Skill 安装方式复制或安装以下目录：

```text
plugins/adaptive-ui-engineer/skills/adaptive-ui-engineer
```

对于扫描 `$HOME/.agents/skills` 的客户端，可复制到该位置。
...
不同客户端的发现和调用方式可能不同。可移植核心只要求 `SKILL.md`；`agents/openai.yaml` 是可选的 Codex 展示扩展。
...
```text
使用 $adaptive-ui-engineer 审计这个页面的响应式和无障碍问题，只读，不修改文件。
```

```text
使用 $adaptive-ui-engineer 简化这个 index 页面，不改框架和兄弟页面，并验证结果。
```

```text
使用 $adaptive-ui-engineer 统一卡片圆角层级，并移除由视口宽度驱动的布局 JavaScript。
```

响应式布局、溢出、视口、圆角、导航、交互无障碍、UI 复杂度和浏览器兼容任务也可以隐式触发。

## 静态审计器

除非显式提供 `--output`，审计器始终只读。
...
Schema 位于 `plugins/adaptive-ui-engineer/skills/adaptive-ui-engineer/assets/audit-config.schema.json`。每个豁免都必须指定窄路径和原因，并会计入输出中的 `summary.suppressed`。
...
- 单元测试和包校验覆盖跨平台路径行为；未来 CI 已配置 Windows、macOS、Linux 和 Python 3.9–3.13。
...
未来 CI 使用固定提交的 Agent Skills 校验器源码，对下载归档执行 SHA-256 校验，并对全部 CI 专用 Python 依赖执行哈希锁定；GitHub Actions 也固定到不可变提交。Codex 作者发布前还应运行当前版本的 Skill Creator 和 Plugin Creator 校验器。
